...
export async function getGames (req,res){ //FIX: como usar vaiável com o % no ILIKE
    const {name} = req.query
    try{
        if(name){
            const games = await db.query(`SELECT * FROM games WHERE name ILIKE '${name}%'`)
...
export async function postGames (req,res){ //FIX: como usar vaiável com o % no ILIKE
    ...
    try{
        const verifyCathegorie = await db.query(`SELECT FROM categories WHERE id=$1`,[categoryId])
        if(verifyCathegorie.rows.length===0){
            res.status(409).send("Categoria inexistente")
            return
        }
        const verifyGame = await db.query(`SELECT FROM games WHERE name ILIKE '${name}%'`)
